...
(
    ...
    `guest_checkout_forbidden` TINYINT DEFAULT 0 NOT NULL COMMENT 'the product may only be bought from an account, so a cart holding it is refused the guest checkout',
    `created_at` DATETIME,
    `updated_at` DATETIME,
    `version` INTEGER DEFAULT 0,
    `version_created_at` DATETIME,
...
(
    ...
    `is_guest` TINYINT DEFAULT 0 NOT NULL COMMENT 'the customer ordered without creating an account and carries no password, until the account is completed',
    `created_at` DATETIME,
    `updated_at` DATETIME,
    `version` INTEGER DEFAULT 0,
    `version_created_at` DATETIME,
