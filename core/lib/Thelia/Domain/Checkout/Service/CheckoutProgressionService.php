 ...
 *
 * Memoized per cart for the request: the delivery check asks the shipping modules what
 * they would quote, which is not something to run twice for one page. The cache is
 * dropped whenever a step is reordered, toggled or created, and on kernel.reset, which
 * is what keeps a runtime serving several requests from one process — FrankenPHP,
 * RoadRunner — from answering the second buyer with the first one's tunnel.
 */
final class CheckoutProgressionService implements EventSubscriberInterface, ResetInterface
{
    ...
    /**
     * The wording to write in the breadcrumb, resolved the way a consent title is: the
     * asked language first, the shop default next, then any language the merchant did
     * write — and the code last. What it never answers is the "DEFAULT TITLE"
     * placeholder I18n forges, which means nothing to a buyer.
     *
     * @throws PropelException
     */
    private function title(string $code, ?CheckoutStep $row, ?string $locale): string
    {
        ...
        $title = (string) I18n::forceI18nRetrieving($locale, 'CheckoutStep', $row->getId(), [])->getTitle();
        ...
        return (string) ($written?->getTitle() ?? $code);
    }
    ...
    private function rowsByCode(): array
    {
        ...
        foreach (CheckoutStepQuery::create()->orderByPosition()->find() as $row) {
    ...
    {
        return (string) ($cart->getId() ?? 'unsaved-'.spl_object_id($cart));
    }
}
